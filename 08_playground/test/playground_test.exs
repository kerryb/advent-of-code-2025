defmodule PlaygroundTest do
  ...
  describe "Playground.run_2/1" do
    test "multiplies the X coordinates of the last two junction boxes that need to be connected" do
      assert Playground.run_2("""
             162,817,812
             57,618,57
             906,360,560
             592,479,940
  ...
  describe "Playground.points/1" do
    test "extracts the points from the input" do
      assert Playground.points("""
             162,817,812
             57,618,57
  ...
  describe "Playground.distances/1" do
    test "calculates the distance between each pair of points" do
      distances = Playground.distances([{162, 817, 812}, {57, 618, 57}, {431, 825, 988}, {739, 650, 466}])
  ...
  describe "Playground.connect_shortest/2" do
    test "removes the shortest pair from the distances map" do
      {distances, _point_1, _point_2, _circuits} =
    ...
    test "returns the points that were connected" do
      assert {_distances, {1, 2, 3}, {2, 3, 4}, _circuits} =
               Playground.connect_shortest(
                 %{
                   {{1, 2, 3}, {3, 4, 5}} => 3.4,
                   {{1, 2, 3}, {2, 3, 4}} => 1.2,
    ...
    test "returns a new circuit for the first pair" do
      assert {_distances, _point_1, _point_2, [[{1, 2, 3}, {2, 3, 4}]]} =
               Playground.connect_shortest(
                 %{
                   {{1, 2, 3}, {3, 4, 5}} => 3.4,
                   {{1, 2, 3}, {2, 3, 4}} => 1.2,
    ...
    test "adds a new circuit if neither of the pair are already in a circuit" do
      assert {_distances, _point_1, _point_2, [[{1, 2, 4}, {3, 4, 5}], [{1, 2, 3}, {2, 3, 4}]]} =
               Playground.connect_shortest(
                 %{
                   {{1, 2, 4}, {3, 4, 5}} => 3.4,
                   {{1, 2, 4}, {4, 5, 6}} => 5.6
    ...
    test "adds the second point to an existing circuit if the first point is already in it" do
      assert {_distances, _point_1, _point_2, [[{3, 4, 5}, {1, 2, 3}, {2, 3, 4}]]} =
               Playground.connect_shortest(
                 %{
                   {{1, 2, 3}, {3, 4, 5}} => 3.4,
                   {{1, 2, 3}, {4, 5, 6}} => 5.6
    ...
    test "combines two circuits if one point is in each" do
      assert {_distances, _point_1, _point_2, [[{1, 2, 3}, {2, 3, 4}, {3, 4, 5}, {7, 8, 9}]]} =
               Playground.connect_shortest(
                 %{
                   {{2, 3, 4}, {3, 4, 5}} => 3.4,
                   {{1, 2, 3}, {4, 5, 6}} => 5.6
    ...
    test "does not change any circuits if both points are already in the same one" do
      assert {_distances, _point_1, _point_2, [[{1, 2, 3}, {2, 3, 4}, {3, 4, 5}, {7, 8, 9}], [{3, 2, 1}, {5, 4, 3}]]} =
               Playground.connect_shortest(
                 %{
                   {{2, 3, 4}, {3, 4, 5}} => 3.4,
                   {{1, 2, 3}, {4, 5, 6}} => 5.6
